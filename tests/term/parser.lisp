...
(test a-title-is-said-once-and-kept
  (let* ((term (a-term :width 10 :height 2))
    ...
    (is (equal "a window" (term:term-title term)))
...
(test a-mode-is-set-and-unset-by-the-same-number
  (let ((term (a-term :width 10 :height 2)))
    ...
    (say term (csi "?2004h"))
    (is (term:term-bracketed-paste term))
    (say term (csi "?2004l"))
...
(test mouse-tracking-modes-are-tracked-not-reported
  (let ((term (a-term :width 10 :height 2)))
    ...
    (say term (csi "?1002h"))
    (is (eq :button-event (term:term-mouse-mode term)))
    (say term (csi "?1003h"))
    (is (eq :any-event (term:term-mouse-mode term)))
    (say term (csi "?1003l"))
    (is (null (term:term-mouse-mode term)))
    (say term (csi "?1006h"))
    (is (term:term-mouse-sgr term))
    (say term (csi "?1006l"))
...
(test screen-column-modes-are-just-tracked
  (let ((term (a-term :width 10 :height 2)))
    ...
    (say term (csi "?45h"))
    (is (term:term-reverse-wraparound term))
    (say term (csi "?2026h"))
...
(test the-cursor-is-the-shape-that-was-asked-for
  (let ((term (a-term :width 10 :height 2)))
    ...
    (say term (csi "4 q"))
    (is (eql :underline (term:term-cursor-style term)))
    (say term (csi "?12h"))
    (is (eql :blinking-underline (term:term-cursor-style term)))
    (say term (csi "6 q"))
...
(test what-a-program-can-make-us-hold-is-bounded
  ...
  (is (eql term::+biggest-param+ (term::held-param (1+ term::+biggest-param+)))
...
(defstruct (a-borrowed-term (:include term:term))
  (rang 0 :type fixnum)
  (marks nil :type list))

(defmethod term:handle-csi ((term a-borrowed-term) (final (eql #\|)) format params)
  "A sequence this library has never heard of."
  (declare (ignore format))
  (push (or (first params) 0) (a-borrowed-term-marks term)))

(defmethod term:handle-csi ((term a-borrowed-term) (final (eql #\J)) format params)
  "A sequence it has, made to mean something else."
  (declare (ignore format params))
  (term:term-write term "kept"))

(defmethod term:handle-esc ((term a-borrowed-term) (final (eql #\7)))
  "And one of its escapes."
  (push :saved (a-borrowed-term-marks term)))

(test a-term-of-your-own-can-add-a-sequence-and-change-one
  (let ((term (term:init-term (make-a-borrowed-term) :width 12 :height 2)))
    (say term (csi "42|"))
...
(defmethod term:term-rang ((term a-borrowed-term))
  (incf (a-borrowed-term-rang term)))

(defmethod term:term-titled ((term a-borrowed-term) title)
  (push (cons :titled title) (a-borrowed-term-marks term)))

(defmethod term:term-linked ((term a-borrowed-term) uri params)
  (push (list :linked uri params) (a-borrowed-term-marks term)))

(defmethod term:term-copied ((term a-borrowed-term) selection data)
  (push (list :copied selection data) (a-borrowed-term-marks term)))

(test a-hyperlink-is-handed-over-not-drawn
  (let ((term (term:init-term (make-a-borrowed-term) :width 10 :height 2)))
...
(test a-method-takes-the-place-of-a-callback-slot
  (let ((term (term:init-term (make-a-borrowed-term) :width 12 :height 2)))
    ...
    (is (equal "a name" (term:term-title term))
