...
(test what-was-rendered-back-to-ansi-is-read-back-the-same
  (let ((term (a-term :width 20 :height 1))
    ...
    (is (equal (term:term-dump-row-string term 0)
               (term:term-dump-row-string again 0)))
    (dotimes (x 20)
      (is (term:face-equal (face-at term x 0) (face-at again x 0))
...
(test a-character-knows-how-many-columns-it-is
  (is (= 1 (term:char-display-width #\a)))
  (is (= 2 (term:char-display-width #\漢)))
  (is (= 0 (term:char-display-width (code-char #x301))))
  (is (= 0 (term:char-display-width #\Nul))))

(test a-presentation-selector-takes-no-column-of-its-own
  (is (= 0 (term:char-display-width (code-char #xFE0E))) "VS15")
  (is (= 0 (term:char-display-width (code-char #xFE0F))) "VS16")
  (is (= 0 (term:char-display-width (code-char #xFEFF))) "zero width no-break space")
  (is (= 0 (term:char-display-width (code-char #x2060))) "word joiner"))

(test yi-and-the-pictographs-below-1f300-are-wide
  (is (= 2 (term:char-display-width (code-char #xA000))) "a Yi syllable")
  (is (= 2 (term:char-display-width (code-char #x1F000))) "a mahjong tile"))

(test cjk-extension-planes-are-wide
  (is (= 2 (term:char-display-width (code-char #x20000))) "extension B")
  (is (= 2 (term:char-display-width (code-char #x2FA1D))) "the compatibility supplement"))

(test an-out-of-range-colour-resets-rather-than-picking-white
  (let ((s (make-string-output-stream)))
    (term:write-sgr (term:make-face :fg 9999) s)
    (is (search ";39" (get-output-stream-string s))
        "an unwritable fg reset it instead of choosing 30-something"))
  (let ((s (make-string-output-stream)))
    (term:write-sgr (term:make-face :bg 9999) s)
    (is (search ";49" (get-output-stream-string s))))
  (let ((s (make-string-output-stream)))
    (term:write-sgr (term:make-face :underline-color 9999) s)
...
(test inverse-is-said-not-worked-out
  (multiple-value-bind (was now) (round-trips (csi "31;44;7m"))
    (is (eql 1 (term:face-fg now)) "the fg that was written is the fg read back")
    (is (eql 4 (term:face-bg now)))
    (is (term:face-inverse now))
    (is (term:face-equal was now))))

(test a-concealed-face-comes-back-concealed
  (multiple-value-bind (was now) (round-trips (csi "31;44;8m"))
    (is (term:face-conceal now))
    (is (eql 1 (term:face-fg now)))
    (is (term:face-equal was now))))
...
(test the-underline-style-survives-the-round-trip
  (dolist (style '((2 . :double) (3 . :curly) (4 . :dotted) (5 . :dashed)))
    (multiple-value-bind (was now) (round-trips (csi "4:~Dm" (car style)))
      (is (eql (cdr style) (term:face-underline now))
          "4:~D came back as ~S" (car style) (term:face-underline now))
      (is (term:face-equal was now)))))

(test blink-and-the-underline-colour-survive-the-round-trip
  (multiple-value-bind (was now) (round-trips (csi "5;4;58;5;9m"))
    (is (eql :slow (term:face-blink now)))
    (is (eql 9 (term:face-underline-color now)))
    (is (term:face-equal was now)))
  (multiple-value-bind (was now) (round-trips (csi "6m"))
    (is (eql :fast (term:face-blink now)))
    (is (term:face-equal was now))))

(test the-nearest-of-the-256-to-an-rgb-colour
  (is (eql 231 (term:rgb-to-color-index 255 255 255)) "white is the top of the cube")
  (is (eql 16 (term:rgb-to-color-index 0 0 0)) "black is the bottom of it")
  (is (eql 244 (term:rgb-to-color-index 128 128 128)) "grey lands on the ramp")
...
(test a-face-that-drops-an-attribute-drops-it
  (let ((term (a-term :width 4 :height 1))
    ...
    (is (null (term:face-bold (face-at again 1 0)))
...
(test a-wide-character-does-not-move-the-columns-after-it
  (let ((term (a-term :width 8 :height 1))
    ...
    (is (equal (term:term-dump-row-string term 0)
               (term:term-dump-row-string again 0))
        "~S came back as ~S" (term:term-dump-row-string term 0)
        (term:term-dump-row-string again 0))))
