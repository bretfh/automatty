...
(test two-panes-side-by-side-each-get-half-the-columns
  (let* ((left (a-pane "aaaa" :rows 3 :cols 10))
         ...
         (tree (vt/ui:row :align :stretch :spacing 0 a b)))
    (laid tree screen)
    (is (eql 0 (vt/ui:left a)))
    (is (eql 10 (vt/ui:width a)) "the left pane was not given half the columns")
    (is (eql 10 (vt/ui:left b)) "the right pane did not start at the halfway mark")
    (is (eql 10 (vt/ui:width b)))
    (is (eql 3 (vt/ui:height a)) "a stretched row did not give it every row")
...
(test the-bar-takes-its-row-off-the-pane-under-it
  (let* ((pane (a-pane "hello" :rows 5 :cols 12))
         ...
         (tree (vt/ui:column :align :stretch v (vt/ui:label "bar"))))
    (laid tree screen)
    (is (eql 4 (vt/ui:height v))
        "the pane was not given the rows the bar left it")
    (is (eql 0 (vt/ui:top v)))
...
(test a-pane-is-drawn-where-it-was-put-and-clipped-to-it
  (let* ((pane (a-pane "0123456789" :rows 2 :cols 10))
         ...
         (tree (vt/ui:row :align :stretch :spacing 0 (vt/ui:gap) v)))
    (laid tree screen)
    (is (eql 5 (vt/ui:left v)) "the two gaps did not split the columns evenly")
...
(test every-pane-in-a-tree-is-found
  (let* ((one (mux:pane-view (a-pane "")))
         ...
         (tree (vt/ui:column (vt/ui:row one) two (vt/ui:label "bar"))))
...
(test a-split-lays-its-panes-out-with-a-rule-between-them
  (let* ((one (a-pane "aaa" :rows 3 :cols 20))
    ...
    (let ((views (mux:views-in tree)))
      (is (eql 10 (vt/ui:width (first views))))
      (is (eql 11 (vt/ui:left (second views)))
