...
(test the-switchboard-is-sessions-as-rows-and-windows-as-boxes-with-their-panes-inside
  (let* ((client (board-client))
         (b (mux::%make-board :starting "todo")))
    (is (equal '("todo" "lib") (mux::board-sessions b client)) "the sessions are not in the server's order")
...
(test the-switchboard-draws-the-server-its-clients-and-every-pane-in-its-window
  (let* ((client (board-client))
         (b (mux::%make-board :starting "todo"))
    ...
    (setf (mux::client-id client) 7
          ...
          (gethash "todo" (mux::client-layouts client)) '((1 "agents" (:across 1 (:down 2 4)) 2 t)))
    (let ((mux::*drawing-for* client))
      (mux:draw-over b screen))
    (let ((all (format nil "~{~A~%~}" (loop :for y :below 30 :collect (shown screen y)))))
      (is (search "◆ server" all) "~A" all)
      (is (search "2 sessions · 2 windows · 4 panes" all) "~A" all)
      (is (search "SESSIONS" all))
      (is (search "CLIENTS" all))
      (is (search "⌨ client you" all) "this client is not on its window: ~A" all)
      (is (search "⌨ client ttys051" all) "the other client is not on its window: ~A" all)
      (is (search "window 1 agents" all) "the window is not titled: ~A" all)
      (is (search "● here" all))
      (is (search "go there" all) "the other session does not say it can be gone to")
      (is (search "> claude" all) "a pane does not say its command: ~A" all)
      (is (search "▲ asks Bash" all) "the asking pane does not say so: ~A" all)
      (is (search "+ window" all))
      (is (search "session todo › window 1 agents › pane arch" all) "the foot has no path: ~A" all)
      (is (search "sessions ↓" all)))
    (is (equal '("todo" 1 1) (mux::board-cursor b)))))

(test the-switchboard-moves-across-windows-and-down-sessions-and-zooms-to-one
  (let* ((client (board-client))
         (b (mux::%make-board :starting "todo")))
    ...
    (let ((mux::*client* client))
      ...
      (is (equal '("todo" 2 7) (mux::board-cursor b)) "right did not go to window 2: ~S" (mux::board-cursor b))
      ...
      (mux::switchboard-zoom)
      (is (equal "lib" (mux::board-zoom b)))
      (mux::switchboard-zoom)
      (is (null (mux::board-zoom b))))))
...
(test the-switchboard-prompts-panes-in-two-sessions-at-once
  (with-server (path :command "cat" :rows 20 :cols 90)
    (with-seer (seer path :rows 20 :cols 90)
      ...
      (is-true (pump seer :want "◆ server") "the switchboard did not open: ~S" (seen seer))
      (is-true (pump seer :want "> cat") "the board has no panes yet: ~S" (seen seer))
      (type-at seer " ")
      (pump seer :seconds 1/4)
  ...
  (with-a-session (session pane server "cat" :rows 8 :cols 100)
      (let ((agent (mux:pane-agent pane)))
        ...
        (let ((said (with-output-to-string (out)
                      (let ((row (mux::window-chip session (mux:session-window session) (mux::now-ms))))
                        (labels ((walk (w)
