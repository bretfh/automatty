...
(defun decode-utf-8-into (decoder octets end out)
  (declare (type (simple-array (unsigned-byte 8) (*)) octets)
           (type fixnum end)
           (type (simple-array character (*)) out))
  (let* ((tail (decoder-tail decoder))
         (held (if tail (length tail) 0))
         (n (+ held end))
         (out (if (>= (length out) n) out (make-string (max n (* 2 (length out))))))
         (at 0)
         (i 0))
    (declare (type fixnum held n at i) (type (simple-array character (*)) out))
    (flet ((byte-at (k)
             (declare (type fixnum k))
             (if (< k held) (char-code (schar tail k)) (aref octets (- k held)))))
      ...
      (loop while (< i n)
            do (let* ((b (byte-at i))
                 ...
                 (declare (type fixnum b took))
                 (cond
                   ((= took 1) (setf (schar out at) (code-char b)) (incf at) (incf i))
                   ((zerop took) (setf (schar out at) +not-a-character+) (incf at) (incf i))
                   ((> (+ i took) n)
                    (let ((rest (make-string (- n i))))
                      (loop for k from i below n
                            for j from 0
                            do (setf (schar rest j) (code-char (byte-at k))))
                      (setf (decoder-tail decoder) rest
                            i n)))
                   (t
                    (let ((code (logand b (case took (2 #x1F) (3 #x0F) (t #x07))))
                          (k 1))
