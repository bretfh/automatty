...
(defmethod measure ((w framed) m aw ah)
  (let ((child (first (parts w))))
    (multiple-value-bind (cw ch)
...
(defmethod lay ((w framed) m x y width height)
  ...
  (destructuring-bind (child top bottom left right tl tr bl br) (parts w)
    (let ((iw (max 0 (- width 2))) (ih (max 0 (- height 2)))
      ...
      (lay left m x (1+ y) 1 ih)
      (lay right m right-x (1+ y) 1 ih)
      (lay tl m x y 1 1)
      (lay tr m right-x y 1 1)
      (lay bl m x bottom-y 1 1)
      (lay br m right-x bottom-y 1 1))))
